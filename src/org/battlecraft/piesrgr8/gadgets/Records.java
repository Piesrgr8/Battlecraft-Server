		...
		if (cmd.getName().equalsIgnoreCase("jukebox")) {
			if (RanksEnum.isAtLeast(p, Ranks.VIPPLUS)) {
				...
				if (args.length == 1) {
					...
					if (JukeboxUtil.isPlaying()) {
						p.sendMessage(Prefix.prefixGadget + ChatColor.RED + "You cannot play any records until the current song is over!");
						return true;
					}
					...
					if (args[0].equalsIgnoreCase("chirp")) {
						try {
							...
							p.sendMessage(Prefix.prefixGadget + ChatColor.GREEN + "Now playing " + ChatColor.YELLOW
									+ "chirp!");
						} catch (Exception e) {
							ErrorUtil.generalError(p, e);
						}
